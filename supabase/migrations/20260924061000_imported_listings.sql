...
-- Some databases (including the live project) already have a source column
-- with a narrower check (user_posted / admin_imported / externally_ingested).
-- Keep those values and allow the importer's source too.
alter table public.listings drop constraint if exists listings_source_check;
alter table public.listings
  add constraint listings_source_check
  check (source in ('user', 'user_posted', 'admin_imported', 'externally_ingested', 'rutgers_off_campus'))
  not valid;
...
alter table public.listings
  ...
  not valid; -- existing rows without a host (older seed data) don't block the migration
